...
if __name__ == '__main__':
    ...
    split = int(0.7*data.shape[0])  # 80-10 break into train-test sets
    trainX = data[:split, :-1]
    ...
    learner = BagLearner(learner=rt.RTLearner, kwargs={"leaf_size": 1}, \
    ...
    rmse = np.sqrt(((Y - trainY)**2).sum() / trainY.shape[0])*60
    ...
    rmse = np.sqrt(((Y - testY)**2).sum() / testY.shape[0])*60
    ...
    for size in range(maxBagSize):
        for i in range(reps):  # average by shuffling & selecting different train/test sets from mc3_p1.data
            np.random.shuffle(data)  # shuffle in-place
            learner = BagLearner(learner=rt.RTLearner, kwargs={"leaf_size": 10}, \
    ...
    plt.plot(range(1, maxBagSize+1), errTrain*60, label='training')
    plt.plot(range(1, maxBagSize+1), errTest*60, label='test')
    plt.xlabel('Number of Tree (00s)')
